...
from config import DATA_ROOT, DEFAULT_SIGNS
...
def load_sign_list(signer_id: str) -> list[str]:
    ...
    if p.exists():
        try:
            ...
            # "global" is the key we use for the shared list
...
def save_sign_list(signer_id: str, signs: list[str]):
    """Write the list under the key 'global'."""
    p = _meta_path()
    # keep any old data (in case we ever add per-signer stuff later)
    if p.exists():
        try:
            data = json.loads(p.read_text())
        except Exception:
...
def add_sign(signer_id: str, new_sign: str):
    """Add to the *global* list – visible to every signer."""
